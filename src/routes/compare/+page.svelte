...
<div class="mx-auto max-w-6xl space-y-6">
  ...
  <Card className="p-4"><div class="grid gap-3 md:grid-cols-[1fr_auto_1fr_auto]"><select bind:value={firstRunId} class="h-9 rounded-md border border-slate-700 bg-slate-950 px-3 text-xs text-slate-300"><option value="">First research</option>{#each readyRuns as run}<option value={run.id}>{run.seedKeyword} · {run.category || 'general'} · {formatDate(run.createdAt)}</option>{/each}</select><div class="hidden items-center justify-center text-slate-600 md:flex"><ArrowRight size={16} /></div><select bind:value={secondRunId} class="h-9 rounded-md border border-slate-700 bg-slate-950 px-3 text-xs text-slate-300"><option value="">Second research</option>{#each readyRuns as run}<option value={run.id}>{run.seedKeyword} · {run.category || 'general'} · {formatDate(run.createdAt)}</option>{/each}</select><Button on:click={compare} disabled={comparing || loading}><GitCompare size={14} />{comparing ? 'Comparing…' : 'Compare'}</Button></div></Card>

<script lang="ts">
  ...
  const indicatorClass = (indicator: string) => indicator === 'violet' ? 'bg-violet-400/10 text-violet-300' : indicator === 'blue' ? 'bg-blue-400/10 text-blue-300' : indicator === 'green' ? 'bg-emerald-400/10 text-emerald-300' : indicator === 'amber' ? 'bg-amber-400/10 text-amber-300' : indicator === 'gray' ? 'bg-slate-700/50 text-slate-400' : 'bg-slate-800 text-slate-500';
  onMount(load);
</script>
...
<div class="mx-auto max-w-7xl space-y-6">
  ...
  {:else if data}
    ...
    {:else}<Card>
      <div class="border-b border-slate-800 px-4 py-3"><p class="text-xs font-medium uppercase tracking-wider text-slate-500">Global rank overview</p><div class="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{#each data.keywords.slice(0, 5) as item}<div class="flex items-center gap-3 rounded-md border border-slate-800 bg-slate-950/60 px-3 py-2"><span class="font-mono text-xs text-slate-500">#{item.globalRank ?? '-'}</span><div class="min-w-0"><p class="truncate text-xs font-medium text-cyan-300">{item.keyword}</p><span class={`mt-1 inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] ${indicatorClass(item.indicator)}`}><span class="h-1.5 w-1.5 rounded-full bg-current"></span>{item.label}</span></div></div>{/each}</div></div>

<script lang="ts">
  ...
  let selectedAssetKeywords = new Set<string>();
  let queueingKeyword = '';
  ...
  async function generateReadout() {
    ...
    aiStatus = '';
    try {
      const hasHistory = (readoutType === 'asset' ? assetReadouts : keywordReadouts).length > 0;
      const result = await api.generateAiReadout(readoutType, { assetType, locale, category, generateAnother: hasHistory });
      ...
      aiStatus = `${readoutType === 'asset' ? 'Asset' : 'Keyword'} readout ${hasHistory ? 'variation baru' : 'selesai'}`;
    } catch (err) {
  ...
  function assetSuggestionKey(item: AssetReadoutItem, keyword: string) {
    return `${item.assetConcept}::${keyword}`;
  }

  function toggleAssetKeyword(item: AssetReadoutItem, keyword: string) {
    const key = assetSuggestionKey(item, keyword);
    const next = new Set(selectedAssetKeywords);
    if (next.has(key)) next.delete(key);
    else if (next.size < 20) next.add(key);
    selectedAssetKeywords = next;
  }

  async function queueAssetKeywords(item: AssetReadoutItem) {
    const suggestions = (item.keywordSuggestions ?? []).filter((suggestion) => selectedAssetKeywords.has(assetSuggestionKey(item, suggestion.keyword))).slice(0, 20);
    if (!suggestions.length) return;
    queueingKeyword = `asset:${item.assetConcept}`;
    try {
      const result = await api.queuePrompts(suggestions.map((suggestion) => ({
        keyword: suggestion.keyword,
        category: category === 'all' ? 'general' : category,
        researchAssetType: assetType === 'videos' ? 'videos' as const : 'images' as const,
        promptOutputType: item.format === 'video' ? 'video' as const : 'image' as const,
        locale: locale === 'all' ? 'en-GB' : locale,
        promptCount: 5,
        recommendedStyle: item.recommendedStyle,
        styleRationale: item.styleRationale,
        sourceReadoutId: latestAsset?.id,
        sourceScore: suggestion.score ?? undefined,
        sourceLevel: suggestion.level,
        sourceConfidence: suggestion.confidence === 'high' || suggestion.confidence === 'medium' ? suggestion.confidence : 'low',
      ...
      selectedAssetKeywords = new Set([...selectedAssetKeywords].filter((key) => !suggestions.some((suggestion) => key === assetSuggestionKey(item, suggestion.keyword))));
      aiStatus = `${result.created.length} asset keyword masuk Prompt Queue · ${result.duplicate.length} duplikat · ${result.rejected.length} ditolak`;
    } catch (err) {
      aiStatus = err instanceof Error ? err.message : 'Asset keyword tidak dapat masuk Prompt Queue';
    } finally { queueingKeyword = ''; }
  }

  async function researchAssetKeywords(item: AssetReadoutItem) {
    const suggestions = (item.keywordSuggestions ?? []).filter((suggestion) => selectedAssetKeywords.has(assetSuggestionKey(item, suggestion.keyword))).slice(0, 20);
    if (!suggestions.length) return;
    queueingKeyword = `research-assets:${item.assetConcept}`;
    try {
      for (const suggestion of suggestions) {
        await api.queueResearch({ keyword: suggestion.keyword, category: category === 'all' ? 'general' : category, assetType: assetType === 'videos' ? 'videos' : 'images', locale: locale === 'all' ? 'en-GB' : locale });
      }
      aiStatus = `${suggestions.length} asset keyword masuk Research Queue`;
      selectedAssetKeywords = new Set([...selectedAssetKeywords].filter((key) => !suggestions.some((suggestion) => key === assetSuggestionKey(item, suggestion.keyword))));
    } catch (err) {
      aiStatus = err instanceof Error ? err.message : 'Asset keyword tidak dapat masuk Research Queue';
    } finally { queueingKeyword = ''; }
  }

...
<div class="mx-auto max-w-7xl space-y-6">
  ...
  <Card className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center"><div class="flex items-center gap-2 text-xs text-slate-500"><SlidersHorizontal size={14} /> Scope</div><select bind:value={assetType} on:change={load} class="h-8 rounded-md border border-slate-700 bg-slate-950 px-2 text-xs text-slate-300 outline-none focus:border-cyan-400"><option value="all">All asset types</option><option value="images">Images</option><option value="videos">Videos</option></select><select bind:value={locale} on:change={load} class="h-8 rounded-md border border-slate-700 bg-slate-950 px-2 text-xs text-slate-300 outline-none focus:border-cyan-400"><option value="all">All locales</option><option value="en-US">English (US)</option><option value="en-GB">English (UK)</option><option value="id-ID">Indonesian</option></select><Button variant="ghost" size="icon" ariaLabel="Refresh insights" on:click={load}><RefreshCw size={15} class={loading ? 'animate-spin' : ''} /></Button></Card>

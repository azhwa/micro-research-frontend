<script lang="ts">
  ...
  import type { AssetType, ResearchRun } from '$lib/types';
  ...
  async function launch() {
    ...
    try {
      ...
      for (const seed of selectedSeeds) {
        const run = await api.createRun({ keyword: seed, category: pack.id, assetType, locale, maxSuggestions, assetsPerQuery });
        created.push(run.id);
      }

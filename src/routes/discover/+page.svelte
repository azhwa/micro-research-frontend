<script lang="ts">
  ...
  async function discoverSeeds(forceNew = false) {
    loading = true; error = '';
    try { discovery = await api.createSeedDiscovery({ topic: topic.trim() || undefined, category, assetType, locale, count, forceNew }); startPolling(discovery.id); }
...
<div class="mx-auto max-w-6xl space-y-7">
  ...
  <div class="grid gap-6 lg:grid-cols-[minmax(0,1.15fr)_minmax(280px,.85fr)]">
    <Card className="p-5 sm:p-6">
      ...
      <form class="mt-6 space-y-5" on:submit|preventDefault={() => discoverSeeds(false)}>
        ...
        <div class="flex flex-col gap-3 border-t border-border pt-5 sm:flex-row sm:items-center sm:justify-between"><p class="text-xs leading-5 text-muted-foreground">AI memprioritaskan single keyword. Maksimal 10% kandidat boleh terdiri dari dua kata.</p><Button type="submit" disabled={loading}>{#if loading}<LoaderCircle size={15} class="animate-spin" /> Finding...{:else}<Compass size={15} /> Find seed ideas{/if}</Button></div>
      </form>
    </Card>

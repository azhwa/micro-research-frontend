...
<div class="mx-auto max-w-4xl space-y-6">
  ...
  <Card className="p-5"><div class="flex items-center gap-2"><KeyRound size={16} class="text-cyan-300" /><h2 class="text-sm font-medium">Add API key</h2></div><p class="mt-2 text-xs leading-5 text-slate-500">Key dienkripsi di backend dan tidak pernah dikembalikan ke browser atau ditulis ke log.</p><form class="mt-5 grid gap-3 sm:grid-cols-[1fr_2fr_auto] sm:items-end" on:submit|preventDefault={addKey}><div class="space-y-1.5"><label for="gemini-key-label" class="block text-xs text-slate-400">Label</label><Input id="gemini-key-label" bind:value={label} placeholder="Personal key" /></div><div class="space-y-1.5"><label for="gemini-key-value" class="block text-xs text-slate-400">Gemini API key</label><Input id="gemini-key-value" bind:value={apiKey} type="password" placeholder="Paste your Gemini API key" /></div><Button type="submit" disabled={saving}><Plus size={15} />{saving ? 'Saving…' : 'Add key'}</Button></form></Card>
